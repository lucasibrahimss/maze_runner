...
std::vector<std::pair<int, int>> encontrarSaidaLabirinto(std::vector<std::vector<char>>& labirinto, int linhaInicial, int colunaInicial) {
	...
	valid_pos.push_back(std::pair<int, int>(linhaInicial, colunaInicial));
...
void print_maze(std::vector<std::vector<char>>& labirinto, std::vector<std::pair<int, int>> caminho) {
	for(int i = 0; i<caminho.size(); i++)
	{
		labirinto[caminho[i].first][caminho[i].second] = '-';
	}
	for (int i = 0; i < num_rows; ++i) {
		for (int j = 0; j < num_cols; ++j) {
...
int main(int argc, char* argv[]) {
	...
	print_maze(initial_pos.labirinto, exit_found);
